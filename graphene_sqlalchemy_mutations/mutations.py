...
class SQLAlchemyMutation(graphene.Mutation):
    ...
    def __init_subclass_with_meta__(
        ...
        if arguments is None and not hasattr(cls, "Arguments"):
            ...
            if meta.create == False:
                arguments["ids"] = graphene.types.List(graphene.ID, required=True)
            # delete
            if meta.delete == False:
                ...
                _out_name = "Delete"
        cls.output = graphene.Field(lambda: SQLAlchemyObjectTypes().get(model), description="")
        cls.ok = graphene.Boolean(description="")
        cls.message = graphene.String(description="")
    ...
    def mutate(cls, self, info, **kwargs):
        ...
        if meta.create:  # new data
            ...
            # model = session.query(meta.model).filter(meta.model.__table__.c[list(meta.model.__table__.primary_key)[0].name] == kwargs["id"]).first()
            model = session.query(meta.model)\
                .filter(meta.model.__table__.c[list(meta.model.__table__.primary_key)[0].name].in_(kwargs["ids"]))\
                .all()
            if not model:
                return cls(output=None, ok=False, message="")
        if meta.delete:  # delete data
            ...
            for single_model in model:
                set_model_attributes(single_model, kwargs["input"])
        try:
            session.commit()
        except SQLAlchemyError as e:
            return cls(output=None, ok=False, message="SQLAlchemyError：%s" % e.args[0])
        except Exception as e:
            return cls(output=None, ok=False, message="Exception：%s" % e.__str__())
        return cls(output=model, ok=True, message="ok")
