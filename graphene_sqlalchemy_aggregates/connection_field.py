...
if MYPY:
    ...
    from .filters import AggregationSet  # noqa: F401; pragma: no cover
...
class AggregationConnectionField(graphene_sqlalchemy.SQLAlchemyConnectionField):
    filter_arg: str = DEFAULT_FILTER_ARG

    factory: 'Union[AggregationFieldFactory, Callable, None]' = None
    filters: dict = {}
    ...
    def __init_subclass__(cls):
        ...
        if cls.filters and cls.factory is None:
            cls.factory = AggregationFieldFactory(cls.filters)
    ...
    @classmethod
    def get_filter_set(cls, info: 'ResolveInfo') -> 'AggregationSet':
        """
        ...
        Returns:
            AggregationSet class from field args.
        ...
        filters: 'AggregationSet' = filters_type.graphene_type
        return filters
...
class ModelLoader(dataloader.DataLoader):
    ...
    @classmethod
    def _get_filter_set(cls, info: 'ResolveInfo') -> 'AggregationSet':
        """
        Get field filter set.
...
class NestedAggregationConnectionField(AggregationConnectionField):
...
class AggregationFieldFactory:
    model_loader_class: 'Type[ModelLoader]' = ModelLoader
    field_class: 'Type[NestedAggregationConnectionField]' = (
        NestedAggregationConnectionField
    )
    ...
    def __call__(
        ...
        filters: 'Optional[AggregationSet]' = self.model_filters.get(model)
